...
public class CheckController {
    ...
    // 전체 조회
    @GetMapping("/member/me/check-list")
    public ResponseEntity getBucket() {

        String email = (String) SecurityContextHolder.getContext().getAuthentication().getPrincipal();

        List<CheckResponseDto> check = checkService.getMyCheck(email);

        return new ResponseEntity<>(new SingleResponseDto<>(check), HttpStatus.OK);
    }
    ...
    // 체크리스트 수정
    @PatchMapping("/check-list/{check-id}")
    ...
    // 체크리스트 삭제
    @DeleteMapping("/check-list/{check-id}")
